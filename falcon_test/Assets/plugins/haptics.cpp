...
HDLServoOpExitCode ContactCB(void* pUserData)
{
	...
	hdlToolButtons(&(haptics->m_buttonActive));
...
void HapticsClass::update()
{
	...
	{
		double offset[3];
		offset[0] = m_newPositionServo[0] - m_positionApp[0];
		offset[1] = m_newPositionServo[1] - m_positionApp[1];
		offset[2] = m_newPositionServo[2] - m_positionApp[2];
	
	double sqrOffset = offset[0] * offset[0] + offset[1] * offset[1] + offset[2] * offset[2];

		if(sqrOffset > 0.06f)
		{
			m_forceServo[0] = (offset[0]) * m_strength;
			m_forceServo[1] = (offset[1]) * m_strength;
			m_forceServo[2] = (offset[2]) * m_strength;
		}
		else
		{
			m_forceServo[0] *= 0.5f;
...
// Interface function to set the new servo speeds
void HapticsClass::setServo(double speed[3])
{
	m_forceServo[0] = speed[0];
	m_forceServo[1] = speed[1];
	m_forceServo[2] = speed[2];
	m_forcedPosition = false;
}

void HapticsClass::setServoPos(double pos[3], double strength)
{
	m_newPositionServo[0] = pos[0];
	m_newPositionServo[1] = pos[1];
	m_newPositionServo[2] = pos[2];
	m_strength = strength;
	m_forcedPosition = true;
}
...
// Interface function to get the current button(s) pressed as an integer
// 0x00000001 = 0001 = Button 1
// 0x00000002 = 0010 = Button 2
// 0x00000004 = 0100 = Button 3
// 0x00000008 = 1000 = Button 4
int HapticsClass::getButtonsDown()
{
	return m_buttonActive;
}

bool HapticsClass::isButton0Down(){
	if ( m_buttonActive & ( 1 << 0 ) )
		return true;
	
	return false;
}

bool HapticsClass::isButton1Down(){
	if ( m_buttonActive & ( 1 << 1 ) )
		return true;
	
	return false;
}

bool HapticsClass::isButton2Down(){
	if ( m_buttonActive & ( 1 << 2 ) )
		return true;
	
	return false;
}

bool HapticsClass::isButton3Down(){
	if ( m_buttonActive & ( 1 << 3 ) )
		return true;
